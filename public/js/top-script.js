...
$('.openbtn').click(function () {
  ...
  $('#header,#container,.container,#footer').toggleClass('mainblur'); //ぼかしたいエリアにmainblurクラスを付与
});
...
$('#g-nav a').click(function () {
  ...
  $('#header,#container,.container,#footer').removeClass('mainblur'); //ぼかしているエリアのmainblurクラスを除去
});
...
function fadeAnime() {
  // 4-1 ふわっ（その場で）
  $('.fadeInTriggerNomal').each(function () {
    //fadeInTriggerというクラス名が
    var elemPos = $(this).offset().top - 50; //要素より、50px上の
    var scroll = $(window).scrollTop();
    var windowHeight = $(window).height();
    if (scroll >= elemPos - windowHeight) {
      $(this).addClass('fadeIn-nomal'); // 画面内に入ったらfadeInというクラス名を追記
    } else {
      $(this).removeClass('fadeIn-nomal'); // 画面外に出たらfadeInというクラス名を外す
    }
  });
